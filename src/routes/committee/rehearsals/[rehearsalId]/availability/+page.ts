...
export const load: PageLoad = async ({ params }) => {
	...
	const userAvailabilities: { name: string; availability: availability | undefined }[] = users
		.filter(
			(user) => user.id !== 'Om9orRRBrIkeXhHTHELj' // Filter out admin account as it is not a user
		)
		.map((user) => {
			return {
				name: user.name,
